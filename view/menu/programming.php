...
if(isset($_GET['search'])){
	$search = $_GET['search'];
	$sql = mysqli_query($Open, "SELECT * FROM post WHERE status_post = 'posted' AND id_category = 1 AND title_post LIKE '%".$search."%' LIMIT $mulai, $halaman");
	$query = mysqli_query($Open, "SELECT * FROM post WHERE status_post = 'posted' AND id_category = 1 AND title_post LIKE '%".$search."%'");
	...
	$sql = mysqli_query($Open, "SELECT * FROM post WHERE status_post = 'posted' AND id_category = 1 ORDER BY id_post DESC LIMIT $mulai, $halaman");
	$query = mysqli_query($Open, "SELECT * FROM post WHERE status_post = 'posted' AND id_category = 1");
	$total = mysqli_num_rows($query);
}
